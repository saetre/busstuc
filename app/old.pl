...
dayModSeqNo(Date,DaySeqNo) :- %% TA-090618
    ...
    decide_period(Date,TTP), %%%%  <----- !!! %% TA-091207 veh_mod(TTP),
...
/*
    TTP:dkodate(EmitDATE,_EMITDAY),
    standardizeemit(EmitDATE,Date1),
*/
...
mod_day_in_set(TTP,DayX,DayCode) :- %% example, May 1=sunday route
...
mod_day_in_set(TTP,DayX,DayCode) :- % NEW regime %% TA-090618
    number(DayX),
    number(DayCode),   
    !,
    TTP:dko(DayCode,_,_,_,_,_,_,MASK),   
...
dayxnumber(_EMITDAY,DayN,XN) :-
    thedayn(DayN,XN).


%% standard day numbers

thedayn(new_years_eve,8). %%  if period starts dec 24
thedayn(new_years_day,9). %%  if period starts dec 24

thedayn(easterday2,8).    %% if period starts palm monday

%% Default daynumbers relative to Emit day

%% day1,..,day14 is day relative to EMITDAY (day1)

thedayn(holiday,1). %% May 17 (1 day periods)
...
/*  OUTDATED
daynr_in_setcode(DayNR,X1111111,Y1111111) :-
   
   bit_no_set_in14(DayNR,X1111111,Y1111111).


bit_no_set_in14(I,X,_):- I >= 1, I =< 7,
    !,
    bit_no_set_in7(I,X).

bit_no_set_in14(I,_,Y):- I >= 8, I =< 14,
    !,
    J is I-7,
    bit_no_set_in7(J,Y).


bit_no_set_in7(I, Z7):- % e.g.   (7  in  0000001)  | (1 in 1000000)
  
     RI is 8-I,        %% e.g.   1                 |  7

     RI1 is RI-1,      %% e.g.   0                 |  6

     pow10(RI,PI),     %% e.g.  10                 |  10000000
     pow10(RI1,PI1),   %% e.g.  01                 |   1000000
    
     Z is mod(Z7,PI),  %% e.g.  1                  |  1000000
     Z1 is Z//PI1,     %% e.g.  1                  |  1
...
regbus(X):-           
     ...
     TT:regbus(X). %% Inclusive set of buses
                   %% in tt/regbusall
*/
...
/*
composite_road(A,B,C):-
 %%     veh_mod(TTP),     %% TA-110318
    composite_road(A,B,C).
*/
...
station(X):- %% TA-110222
    myflags(actual_domain,FARA), 
    domain_module(FARA,TTP),
    TTP \== nil, 
    modstat(TTP,X).    
...
route(A,B,C):-         bus_mod(TTP),TTP:route(A,B,C).
...
% depnode(TimeArr,TimeDep,DelayArr,DelayDep,BegTime,Rid,Station)



%% TIME PARAMETERS %% Moved to busdat.pl
...
streetstation2(Ident,Num,Station):- %% Station name in streetstat
...
stationsat(_Place,Neigh,Stations) :- % Special case, Neigh is also a neighbourhood
    ...
    !,
	 set_of(Station,  bingbong(Neigh,Station), %% (place_station(Neigh,Station); isat(Station,Neigh)), 
           IsatStations),
    Stations=IsatStations. 
...
occurs_afterwards(depnode(_A1,D1,_,_,BegTime,Rid,Stat1),EndDeps):- 

     D1 \== 9999, %% non existing departure time 

     member( depnode(A2,_D2,_X2,Y2, BegTime,Rid,Stat2), EndDeps), 

     Y2 > 0, %% then never after (Bus 36 M4 -> Jakobsli -> M4) %% TA-090627
...
nextdep(Tid,Depnode,Deps):-
   Depnode=depnode(_,Time9,_,_,_,_,_),   %% <- next DEPARTURE
...
pass_between(depnode(ArrTime,_DepTime,_,_,_,_,_),LowTime,HighTime):- 

    ArrTime >= LowTime, %% There is always an arrival time 
    ArrTime =< HighTime. 


arr_between(depnode(Time0,_Time9,_,_,_,_,_),LowTime,HighTime):- 
    LowTime =< Time0,
    Time0 =< HighTime. 

dep_between(depnode(_Time0,Time9,_,_,_,_,_),LowTime,HighTime):- 
    LowTime =< Time9,
    Time9 =< HighTime. 


not_arr_between(depnode(Time0,_Time9,_,_,_,_,_),LowTime,HighTime):- %% TA-110322
    LowTime > Time0
    ;
    Time0 > HighTime. 

not_dep_between(depnode(_Time0,Time9,_,_,_,_,_),LowTime,HighTime):- %% TA-110322
    LowTime > Time9
    ;
    Time9 > HighTime. 
...
pass_before(depnode(Time0,_,_,_,_,_,_),LimTime):- 
	 Time0 =< LimTime.


% pass_before(depnode(_,Time9,_,_,_,_,_),LimTime):- 
%	 Time9 =< LimTime.
...
pass_after(depnode(_,Time9,_,_,_,_,_),LimTime):- 
...
arr_after(depnode(ArrTime,_,_,_,_,_,_),LimTime):- 
    ArrTime  >= LimTime.



dep_after(depnode(_,DepTime,_,_,_,_,_),LimTime):- 
    DepTime  >= LimTime.


arrdep_before(depnode(ArrTime,DepTime,_,_,_,_,_),LimTime):-   %% TA-110308
   ( ArrTime = 9999 -> DT=DepTime;DT=ArrTime ),               %% ad hoc arrival before time,
    DT  =< LimTime.                                  



arrdep_after(depnode(ArrTime,DepTime,_,_,_,_,_),LimTime):- 
...
pass_after_strict(depnode(_,Time9,_,_,_,_,_),LimTime):- 
    Time9 \== 9999, %% non existing dep time 
	 Time9 > LimTime.
...
pass_at(depnode(Time,_,_,_,_,_,_),LimTime):- 
    Time=LimTime.          
...
pass_bus(depnode(_,_,_,_,_,Rid,_),Bus):-
	 ridtobusnr(Rid,Bus). 
...
diffdep4(StartPlace,EndPlace,Deps,Diff):-
	 member(depnode(Time1,_,_,_,BegTime,Rid,Station1),Deps),
...
departure(Bus,Place,Day,DepSet) :- % Bussavgangene for en buss ved en stasjon
   ...
   !,  
	veh_mod(TTP),
   set_of(depnode(Time0,Time9,DelArr,DelDep,BegTime,Rid1,Station1),
	      depbusMOD(TTP,Bus,Place,Day,Time0,Time9,DelArr,DelDep,BegTime,Rid1,Station1),
	      DepSet).

%% Double departures Hastus   Friday 2800 + Staurday 0400
not_extreme_hastus_time(X):- 
    X=depnode(_2820,_,_20,_,T2800,_bus_108_3049,_stavset),
    T2800 < 1200.
...
avoidnightbustoplace(Place,Y) :-
    Y= depnode(_,_,_,_,_,Bus_57_nn,_),
...
setdepMOD(TTP,Place,Kay,DepSet):-
	set_of(depnode(Time0,Time9,DelArr,DelDep,BegTime1,Rid1,Station1),
	      depMOD(TTP,Place,Kay,Time0,Time9,DelArr,DelDep,BegTime1,Rid1,Station1),
	      DepSet).


day_route(depnode(_Time0,_Time9,_DelArr,_DelDep,_BegTime,Rid1,_Station1)):-
    ridtobusnr(Rid1,NB),
    \+ nightbus(NB),
    \+ exbusname(NB,skolebuss). %% Ad Hoc %% TA-090622 




night_route(depnode(_Time0,_Time9,_DelArr,_DelDep,_BegTime,Rid1,_Station1)):-
    ridtobusnr(Rid1,NB),
    nightbus(NB).


airbus_route(depnode(_Time0,_Time9,_DelArr,_DelDep,_BegTime,Rid1,_Station1)):-
    ridtobusnr(Rid1,NB),
    airbus(NB). %% TA-090331
...
depMOD0(TTP,Place,Time0,Time9,Rid,Station) :-
   passMOD(TTP,Rid,Place,_STATNO,Station,DelArr,DelDep),   

   adjustarr999(DelArr,DelDep,RealDelArr),  
   adjustdep999(DelArr,DelDep,RealDelDep), 

	xdepartureMOD(TTP,Rid,_,BegTime,_Kay),  
...
%% Bus is unknown
depMOD(TTP,Place,Day,Time0,Time9,DelArr,DelDep,BegTime,Rid,Station) :-
   passMOD(TTP,Rid,Place,_STATNO,Station,DelArr,DelDep),   

%%%%%   (Rid=bus_60_74 -> output(Rid);true),  %%%% <--- panic
   ...
   mod_day_in_set(TTP,Day,Kay),   
	addtotime(BegTime,RealDelArr,Time0),   
   addtotime(BegTime,RealDelDep,Time9),
   true.

%%%%%      (Rid=bus_60_74 -> output(Time9);true).  


%% Bus is given
depbusMOD(TTP,Bus,Place,Day,Time0,Time9,DelArr,DelDep,BegTime,Rid,Station) :-
	passMOD(TTP,Rid,Place,_STATNO,Station,DelArr,DelDep),
...
/** SUPERFLUOUS %% TA-110302
alias_stationx(Station,Place):- %% to be used days before AtB routes
         %% regarding AtB routes
    myflags( query_period,team),
    myflags( application_period,atb),
...
/* unnec %% TA-100824
place_station(Place,RBSK):-  %% rosenborg_skole Team
    myflags( query_period,team),
    myflags( application_period,atb),
    bound(Place),               %% rosenborg_sk
    aliasteamatb(_,Place,RBSK). %% means not a station in Team
*/
...
place_station(Ident,Station):-	%% Name in streetstat
	 bound(Ident),
	 streetstat(Ident,_,_,_,Station),
...
%  connections(StartDeps,EndDeps,Bus,FromPlace,ToPlace,Day,SeqNo,Opts,Deps,Mid01)
...
connections(StartDeps,EndDeps,Bus,FromPlace,ToPlace,Day,SeqNo,Opts,Deps,Mid01):-
    ...
    nonvar(StartDeps), 
    nonvar(EndDeps),
    remove_spurious_deps(StartDeps,StartDeps0), 
    
    remove_roundtrip_deps(FromPlace,ToPlace,StartDeps0,StartDeps1), %% TA-110224
 
    remove_spurious_arrs(EndDeps,EndDeps1), 
    
    coupled(StartDeps1,EndDeps1,Bus,FromPlace,ToPlace,Day,SeqNo,Opts,Deps,Mid01), 
...
looping(FromPlace,ToPlace,Bingo):- %% TA-110224
        Bingo =depnode(9999,T1520,_DelArr,_DelDep,_Time,RID,FromPlace),

%%%     RID=bus_2_532,
%%%     FromPlace=hamar_skysstasjon, 
%%%    !,
%%%        traceprog(4,('***',Bingo)),
%%%      bingoloop,
...
remove_spurious_deps(Deps,Deps1):-
    set_filter(X, (member(X,Deps),\+ (X=depnode(_,_,_,999,_,_,_))),Deps,Deps1).
...
coupled(StartDeps,EndDeps,_Bus,FromStat,ToStat,Day,SeqNo,Opts,Dep01,Mid01) :- 
...
%%%%%%%%    kindofday(Day,Kay), 
    ...
    findquicktransfers(Opts,FromStat,ToStat,FT1,LT2,Day,SeqNo,SetOfDepMid), 
   ...
   transferXYZ(FromStat,ToStat,TransferStations),
    set_of(depX(ArrT,Dep01,Mid01),   % sort on final arrival time
      ( member(X,TransferStations),
        findfirstquicktransfer(FromStat,ToStat,X,FT1,LT2,Day,SeqNo,Dep01,Mid01), 
...
coupled(StartDeps,EndDeps,_,_,_,Day,SeqNo,Opts,Deps,Mid01) :- 
...
%%%%%%%%%%%%%%	myflags(smsflag,true), %% NB no bygone departs
   ...
   coupled_time(BothStartDeps,StartDeps1,EndDeps1,Day,SeqNo,Opts,Deps,Mid01).
...
coupled(StartDeps,EndDeps,_,_,_,Day,SeqNo,Opts,Deps,Mid01) :- 
	...
	keepcorr(StartDeps,EndDeps,BothStartDeps),
   !,
   coupled_time(BothStartDeps,StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01).
...
coupled_time(BothStartDeps,StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01):- 
    ...
    timeout(            %%  calls time_out if not notimeoutflag
      coupled1(BothStartDeps,StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01), 
        MAXTIME, %% 10 seconds
        Success),
...
%%%%%%%%%%    !,
    ...
    coupled2(BothStartDeps,LeanStartDeps,LeanEndDeps,Day,SeqNo,Opts,Deps,Mid01). 
...
%%%%     testmember(nextaftertime(_),Opts),  %% ONLY IF EXPL TIME %% TA-100828
                                          %% TA-100629
     ...
     firstmem(BothStartDeps, FirstDirect),  % First opportunity for direct route
     deptimeof(FirstDirect,FirstDirectTime),

     lastmem(BothStartDeps, LastDirect),  % Last opportunity for direct route
     arrtimeof(LastDirect,LastDirectTime),

     irrelevantdirect(Opts,FirstDirectTime,LastDirectTime,FirstStartTime),

     progtrace(4,coup1),                        

%%   !, %% dont exclude  direct connection
...
%% FIRSTFREAK
%% IF SMSFLAG, FIRSTCORR  There is a direct connection, but FIRST EnDep >> First Startdep
%% ==> Try indirect
...
coupled2(BothStartDeps,StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01) :-
     ...
     once1(     (member(last(_),Opts) ; member(direct,Opts))), % testmember doesnt work  
 
    \+ member(nextaftertime(_),Opts),  %% IF NO MORE, dont give past deps %%?
 
     lastmem(BothStartDeps,DepEarly),  %% nb L X

     DepEarly= depnode(_TimeArr,TimeDep,_DelayArr,_DelayDep,_BegTime,_Rid,_Station),

     TimeDep < 2100,  %% Rosendal til Risvollan
...
coupled2(BothStartDeps,StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01) :-
     ...
     myflags(smsflag,fuck), %% <--- %% some error !

    \+ member(direct,Opts), 
 
    \+ member(nextaftertime(_),Opts),  %% IF NO MORE, dont give past deps %% ?
 
    %% dont allow 5 hours waiting even if direct

     \+ member(last(_),Opts),  
     \+ member(lastcorr,Opts),  

     BothStartDeps \== [], %% even if there is a direct connection ( earlier)

     sameday(Day), 

     timenow2(0,NOW),

% there is a direct connection, but it is irrelevant
                
     keepwithin(NOW,60, BothStartDeps,BAF),
     BAF == [],  %% no direct connection between now and 60 min
     
     keepwithin(NOW,60,StartDeps,AfterStartDeps), %% only within 60 m // Unnec
     ...
     coupled3(AfterStartDeps,AfterEndDeps,Day,SeqNo,Opts,Deps,Mid01).




%% No cuts here, also try first in the morning if none whatsoever  after now 

%% IF  time is given,  dont wait indefinitely
...
% % % Pick the first time from Startdeps (Assume keepafter is run)     

     firstmem(StartDeps,D), deptimeof(D,FIRSTTIME), 
%%   StartDeps = [S1|_],    S1= depnode(_,FIRSTTIME,_,_,_,_,_),

% % % % 

% there is a direct connection, but it is irrelevant
                
     keepwithin(FIRSTTIME,60, BothStartDeps,BAF), %    keepafter(NOW,BothStartDeps,BAF),
     BAF == [],  %% no direct connection between now and 60 min
     
     keepwithin(FIRSTTIME,60,StartDeps,AfterStartDeps), %% only within 60 m // Unnec
     keepwithin(FIRSTTIME,60,EndDeps,AfterEndDeps),     %% 
...
coupled2(BothStartDeps,_,_,Day,_SeqNo,Opts,BothStartDeps1,MidDeps) :-
    BothStartDeps \== [],
    \+ testmember(time,Opts), %% 2 timer siden %% <--- ## ? %% TA-100630
    %%%%%%    myflags(smsflag,true), %% TA-090330
    sameday(Day), 
    \+ testmember(lastcorr,Opts), %% set by previous bus %% TA-091104
%%%%%%%%%    !,                            %% unnec otherwise (or informative)
    progtrace(4,coup6),   
    timenow2(0,NOW),
...
coupled2(BothStartDeps,_,_,_Day,_SeqNo,_opts,BothStartDeps,MidDeps) :- 
    BothStartDeps \== [],
    !,
    progtrace(4,coup7), 
    MidDeps=[].
...
%% if no other time options than ...,  try next 
   (set_difference(Opts,[first,first(1),next,next(_)],Opts1),
    ...
    coupledtry(AfterStartDeps,AfterEndDeps,Day,SeqNo,Opts,Deps,Mid01).
...
coupled2([],StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01) :-
    ...
    sameday(Day),
	 !,
	 progtrace(4,coup9),     
    coupledtry(StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01).
...
coupled2([],StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01) :- 
    ...
    \+ sameday(Day), 
	 !,
	 progtrace(4,coup10),        
	 coupled3(StartDeps,EndDeps,Day,SeqNo,Opts,Deps,Mid01).
...
(    FirstDirectTime > F60; %% even after 60 minutes, have to wait
     LastDirectTime < FirstStartTime). %% direct only in the past




firstactualdeparturetime(Day,_SeqNo,S,FAT) :-
     myflags(smsflag,true),
     sameday(Day),
     !,
     timenow2(0,NOW),
...
firstactualdeparturetime(_Day,_SeqNo,S,FAT) :-
     firstdeparture(S,X),
     deptimeof(X,FAT).
...
coupledtry(StartDeps,EndDeps,Day,SeqNo,Opts,Dep01,Mid01) :-
   ...
   \+  time_options(Opts), 
    ...
    keeparrafter(NOW,EndDeps,AfterEndDeps), 
	 ...
	 coupled3(AfterStartDeps,AfterEndDeps,Day,SeqNo,Opts,Dep01,Mid01), 
	 Dep01 \== [],
    !. 
...
coupledtry(StartDeps,EndDeps,Day,SeqNo,Opts,Dep01,Mid01) :-
	 coupled3(StartDeps,EndDeps,Day,SeqNo,Opts,Dep01,Mid01),
 	 !.  % If using the whole set fails, then there is no hope
	


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% Finner beste kobling mellom sett  % StartDeps =[depnode(_,StartTime,_,_,_,_,_)|_], %% ?

coupled3(StartDeps,EndDeps,Day,SeqNo,Opts,Dep01,Mid01) :- 

	bestcorr(StartDeps,EndDeps,Day,SeqNo,

				depnode(_,Time1,_,_,_,Rid1,Station1), % Start
				depnode(Time2,_,_,_,_,Rid2,Station2), % End 
				[OffTime,OffStation,OnTime,OnStation],Opts), %% List
	ridtobusname(Rid1,BusN1), 
	ridtobusname(Rid2,BusN2), 

   progtrace(5,depnode(_,Time1,_,_,_,Rid1,Station1)), %% TA-091007
   progtrace(5,depnode(Time2,_,_,_,_,Rid2,Station2)), %%
...
bestcorr(StartDeps,EndDeps,Day,SeqNo, StartDep,EndDep,Mid,Opts) :-
	 tofindlastcorr(Opts),
		StartDep  =		depnode(_,_,_,_,_,_,Orig),
		EndDep    =		depnode(_,_,_,_,_,_,Dest),
	 reverse(StartDeps,RevDeps),
	 lastcorr(Orig,Dest,RevDeps,EndDeps,Day,SeqNo,StartDep,EndDep,Mid).
...
lastcorr(Orig,Dest,StartDeps,EndDeps,Day,SeqNo,StartDep,EndDep,Mid) :- 
...
% before first EndDep
    ...
    islastcorr(StartDep,Orig,Dest,EndDeps,Day,SeqNo,EndDep,Mid).
	 

islastcorr(DEP,Orig,Dest,InDeps,Day,SeqNo, StartDep,Mid) :-
	 ...
	 islastcorr2(Orig,Dest,Deps,Day,SeqNo,DEP,Mid,StartDep).

islastcorr2(Orig,Dest,DepList,Day,SeqNo,EndDep,Mid,Dep) :-
	 member(Dep,DepList),
    iscorr3(Orig,Dest,Day,SeqNo,EndDep,Dep,Mid).


%%% firstcorr(Orig,Dest,+StartDeps,+EndDeps,Day,SeqNo,-StartDep,-EndDep,-Mid01) 
...
firstcorr(Orig,Dest,StartDeps,EndDeps,Day,SeqNo,StartDep,EndDep,Mid):- 
    ...
    isfirstcorr(EndDep,Orig,Dest,Day,SeqNo,StartDepsRev,StartDep,Mid). 



arrtimeof(depnode(X,Time0,_,_,_,_,_),Time):-X=9999,!,Time=Time0. 
arrtimeof(depnode(Time0,_,_,_,_,_,_),Time0).

deptimeof(depnode(_,Time0,_,_,_,_,_),Time0). %% (always?)
...
firstdeptime(Time0,[depnode(_,Time0,_,_,_,_,_)|_]).

lastdeptime(Time0,Z):-   
    reverse(Z,Z1),
    firstdeptime(Time0,Z1).


firstarrtime(Time,[depnode(X,Time0,_,_,_,_,_)|_]):-X=9999,!,Time=Time0. 
firstarrtime(Time0,[depnode(Time0,_,_,_,_,_,_)|_]).


lastarrtime(Time0,Z):-   %% 
    reverse(Z,Z1),
    firstarrtime(Time0,Z1).
...
firsttime(Time,[depnode(X,Time0,_,_,_,_,_)|_]):-X=9999,!,Time=Time0. 
firsttime(Time0,[depnode(Time0,_,_,_,_,_,_)|_]).
...
isfirstcorr( EndDep,Orig,Dest,Day,SeqNo,StartDepsRev,StartDep,Mid) :-
    arrtimeof(EndDep,Time0),  %%    <---- ARR
	 isfirstcorr2(Time0, EndDep, Orig,Dest,Day,SeqNo,StartDepsRev,StartDep,Mid).

%%%%% EXPERIMENTAL  %% TA-100901

isfirstcorr2(Time0,EndDep,Orig,Dest,Day,SeqNo,DepList,StartDep,Mid) :- 
...
%%                     47   46   Klæbu Sandmoen CitySyd
    ...
    iscorr3(Orig,Dest,Day,SeqNo,StartDep,EndDep,Mid),
...
iscorr3(_Orig,_Dest,_Day,_SeqNo,StartDep,EndDep,Mid):- 
    ridof(StartDep,Rid1),
    ridof(EndDep,Rid2),
    Rid1==Rid2, % You have despite expectations found a (late) direct transfer
    !,
    Mid = [0,nil,9999,nil].  %% Make a dummy transfer
 

iscorr3(Orig,Dest,_Day,DaySeqNo,StartDep,EndDep,Mid):- 
    StartDep = depnode(_OrigArrTime,OrigDepTime,_DelArr1,_DelDep1,_BegTime1,Rid1,_),
    EndDep   = depnode(DestArrTime,_DestTime9,_DelArr2,_DelDep2,_BegTime2,Rid2,_),
 ...
 \+ toolongtraveltime(OrigDepTime,DestArrTime), 
    ...
    passtimeMOD(TTP,Rid1,_,Orig,      Seq1,_,      _,  DaySeqNo), 
    passtimeMOD(TTP,Rid1,_,OffStation,Seq2,OffTime,_,  DaySeqNo),
 
    Seq2 > Seq1,
 
    corresponds(OffStation,OnStation),

    passtimeMOD(TTP,Rid2,_,OnStation,SeqA1,_,OnTime,DaySeqNo),

    ridtobusname(Rid1,Bus1),
    ridtobusname(Rid2,Bus2),


%%%    trytransbuslist(Bus1,Bus2,OffStation,OnStation),%% check if on transbuslist %% TA-110318


    compute_delay_margin(Bus1,Bus2,OffStation,OnStation,Marg),
    addtotime(OffTime,Marg,OffTime2), %% 5 minutes (busdat1.pl)

    OnTime >= OffTime2,
    passtimeMOD(TTP,Rid2,_,Dest,SeqA2,DestArrTime,_,DaySeqNo),
    SeqA2 > SeqA1,
    OnTime =< DestArrTime,     % Check Less Dest Time 
...
%% Day has already been is checked %% TA-110318
...
% statbus/busstat are pertinent to the current module of the creation date !
