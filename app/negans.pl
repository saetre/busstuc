...
makenegative((_,_),Q,Mess):- %%  passesstation keepbetweenstat
   ...
   ( neverpasses(Nine,Lade1) -> 
	    (Mess = (bcpbc(Bus),bcp(Nine),bcp(neverpasses),bwr(Lade1),period));

    neverpasses(Nine,Berg1) -> 
	      (Mess = (bcpbc(Bus),bcp(Nine),bcp(neverpasses),bwr(Berg1),period))),
    progtrace(4,ne10),!.
...
makenegative((_,_),Q,Mess):- 
   ...
   ( neverpasses(Nine,Lade1) 
     -> 
       ...
       (getactualday(Q,Day),
       (Mess= (bcpbc(notpossible),ondays(Day),period, standnight(Day)))) 
...
makenegative((which(_),_),Q,Ans)   :- 
    ...
    progtrace(4,ne12),!,
    getactualday(Q,Day),
    (Ans= (bcpbc(notpossible),ondays(Day),period,standnight(Day))). 
...
%   sequence_member(departure(Nine,Lade,_,_),Q), %% does not catch keepbus %%
    ...
    progtrace(4,ne16),!,
    Mess = ( bcpbc(cannotfindanyroutes),period). 
          %%,  bcpbc(Bus),bcp(Nine),bcp(neverpasses),bwr(Lade),period). %% given before



makenegative((_When,_),Q,space0):- %% 
...
makenegative((explain,_),Q,Ans)    :- 
        sequence_member(departure(_,_,_,_),Q),
         progtrace(4,ne39),!,
        Ans=(bcpbc(it),space,bcp(is),bcp(notpossible),period). 
...
makenegative((explain,_),Q,Ans)    :- 
   ...
   progtrace(4,ne40),!,      
   Ans= ( bcpbc(cannotanswer),nl). %% ,period). %% in addition to messages


makenegative((howmany(_),_),Q,Ans) :- 
  \+ sequence_member(message(idonotknow),Q), %% Avoid double
   progtrace(4,ne41),!,  
     Ans= (bcpbc(idonotknow),nl). %% honest  ( not none) 
...
makenegative((test,P),Q,Ans)  :- 
 ...
 \+ sequence_member(dob/say/tuc/_/_,P), %% ad hoc-> cannot
    ...
    Ans=(bcpbc(idonotknow),nl).   %% know is knowledge
...
makenegative((test,P),_,Ans)  :- 
    ...
    A \== B, A \== it, B \== it, 
     progtrace(4,ne43),!,
    Ans=(bcpbc(no),period).       %% To know what you don't
...
makenegative((doit,reply(_M)),_,Ans)  :-
   progtrace(4,ne46),!,
   Ans= space0. %% output(_M). %% avoids double
...
makenegative(_P,Q,Ans)  :-  %% never for new, do
     \+  sequence_member(message(_),Q),
     Ans=(bcpbc(cannotanswer),nl), %% TA-100915  hotell til IKEA 
     progtrace(4,ne52),!.


makenegative(_,_,true) :- %% no extra nl
...
notthenanswer(_Date,Day,Clock,Q,(bcpbc(nolonger),nibcp(Day),AttimeClock,DirAns,standnight(Day))):- 
...
notthenanswer(_Date,Day,_,Q, (bcpbc(cannotfindanyroutes),nibcp(Day),DirAns,standnight(Day)) ):- 
...
notthenanswer(_Date,Day,Clock,Q, (bcpbc(noroutes),nibcp(Day),bcp(attime),bwt(Clock),DirAns,standnight(Day)) ):-
...
(  %%%%%%   myflags(smsflag,true);  %% Will "always" try solutions after now 
     member(next,Options);
...
notthenanswer(_Date,Day,_,Q,(       bcpbc(nolonger),nibcp(Day),DirAns,standnight(Day))):- 
     sequence_member(connections(_,_,_,_,_,_,_,Options,_,_),Q),  %% not test 
(    myflags(smsflag,true);  %% dubious, excludes nolonger message if smsflag false
     member(next,Options);
     member(next(_),Options)),
...
notthenanswer(_Date,Day,_,Q, 
    (bcpbc(nolonger),  %% no time, no other date
...
notthenanswer(Date,Day,_,Q, 
    (bcpbc(notpossible), ondate(Date),%% TA-110301
                DirAns,standnight(Day))):- 
    !,
    progtrace(4,nt11),  
    dirans(Q,DirAns).
