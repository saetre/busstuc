...
class LDAPSearcher {
    ...
    private LDAPSearcher(String strSearch) {
        ...
        env.put(Context.PROVIDER_URL, MY_HOST ); // endret av bruland
        ...
        try {
            ...
            env.put(Context.PROVIDER_URL, MY_HOST ); // endret av bruland
            // env.put(Context.PROVIDER_URL, "ldap://ldap.ntnu.no/o=hist,c=no");
      /*
      fjernet av bruland
      m_ctxHiST = new InitialDirContext(env);
      env.put(Context.PROVIDER_URL, "ldap://ldap.ntnu.no/o=sintef,c=no");
      m_ctxSINTEF = new InitialDirContext(env);
      env.put(Context.PROVIDER_URL, "ldap://ldap.ntnu.no/o=andre,c=no");
      m_ctxOther = new InitialDirContext(env);
       */
    ...
    private void tag(String iStr) {
          ...
          /*
    String tagwords[] = new String[words.length-1];
    System.arraycopy(words,1,tagwords,0,words.length-1);

    for (int i=0;i<words.length-1;i++) {	// bruland
                tagwords[i]=konvInn(tagwords[i]);
        }
           */
        ...
        while (st.hasMoreTokens()) {
            ...
            //System.out.println(words[teller]);
            teller++;
        }
    ...
    private void srch()  throws IllegalArgumentException {
	    ...
	    errTrace("new string: "+m_strSearch, 0);
       ...
       */
        ...
        for (int i = 0; i < astrWhereFields.length; i++) {
            ...
            || astrWhereFields[i].equals("lastname")) {
                ...
                if ((strMailSearch = isMailDep(astrWhereVals[i])) != null) {
					...
					errTrace("ikke emaildep", 1);
                    depList.addAll(getDepList(astrWhereVals[i]));
                    if (depList.size()>0){
						String depSrch = strMakeSearchString(astrWhereVals[i], "ou");
						errTrace("ting: "+depSrch, 1);
						errTrace("ting etter strStripQuotes: "+strStripQuotes(depSrch), 1);
        ...
        /*errTrace("depsize. "+depList.size(), 1);
        for (int i=0; i<depList.size(); i++){
			errTrace("endep: "+(String)depList.get(i), 1);
		}*/
        ...
        if (depListing){
			...
			while(i<2 && iResCount==0){
				if (depList.isEmpty()){
					...
					while(it.hasNext()){
						SearchRes sr = performeSearch(strSrch, (String)it.next(), countLimit, strFields);
						strXML += sr.m_strXml;
						iResCount += sr.m_iCountRes;
					}
		...
		if (st.countTokens()==2){
			...
			for (int i=0; i<depList.size(); i++){
				...
				//if (iElementCount <= 10){
					...
					//str = XMLTagger.strTag("ou","ntnu"); // strOu0
					str="";
 ...
 /*           String grepcmd[] = new String[] {
                "grep",
                        "-i","-e",
                        strStripQuotes(strKey),
                        "depList.txt"
            };

            if (TRACEVAL > 2){
                for (int ii = 0; ii < 5; ii++)
                    System.err.print(grepcmd[ii] + " ");
                System.err.println();
            }
            ...
            while ( (strLine = bfr.readLine()) != null) {
                errTrace("Grep result: " + strLine, 2);
                Matcher m1 = p1.matcher(strLine);
                if (m1.find())
                    ret.add( (Object) createDnFromCode(m1.group(1)));
            }

            int iExitVal = pGrep.waitFor();
            errTrace("grep closed with exit status " + iExitVal,2);
