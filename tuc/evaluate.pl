...
%% META PREDICATES : for/2, foralltest/2, set_of/3, set_ops/3, test/1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
new_focus(X,M) :-      %  Focus is allowed in permament store
    ...
    retractall( main:difact(UID,_ is_the M) ),  %% difact/1 in facts, difact/2 is virtual in main.pl?
    retractall( main:fact0(_ is_the M) ),
    ( (permanence =: 1) ->         %% utility.pl
       asserta( main:fact0(X is_the M) );  
       asserta( main:difact( UID, X is_the M) ) ). %%   % Last Qualified Reference
...
fakt(X):-
         difact(X); %% dynamic          From facts.pl
         fact0(X). %% static
