...
% UNIT: /tuc/
:- module( facts, [ difact/1, fact/1, have/4, isa/2, neighbourhood/1, precedent_firstname/1 ] ).
        %% , unproperstation1/1 %% RS-150111. Broken, endless recursive!
        % Unknown stations (unproperstations) could be neibhorhoods at least!
...
:- use_module( slash, [ def/1, description/2 ] ). %% RS-131228

%:- use_module( tuc:world0, [ area/2 ] ).   %% RS-131225
...
%% RS-111205, UNIT: db/
:- use_module( '../db/auxtables', [ unproperstation/1 ] ).  %% RS-140928 Auto-generated by makeauxtables! Moved down
...
:- use_module( '../db/places', [ isat/2, nostation/1, placestat/2, underspecified_place/1, unwanted_place/1 ]). %% RS-131225
:- use_module( '../db/regbusall', [ nightbus/1, regbus/1 ] ). % , regbus/1 ] ). %% RS-140416   % regbus/1 moved to busdat (Period independent - Merge? )
...
fact( X isa Y ) :- !,      %  Default type definitions
        X isa Y .      %% RS-140914, Supercrash here!
%        facts:( X isa Y ).
...
%unproperstation1( X ) :-
%        %% RS-150104 This should be done (semi-) automatically right after makeauxtables:makeauxtables/0 is done...
%        % use_module( 'db/auxtables', [ unproperstation/1 ] ), 
%    unproperstation(X), %% no bus passes %%  maybe empty
%    \+ neighbourhood(X).  %% LOOPS BACK!?!
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% BAD SECTION IN PROLOG 4.2.3 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %% RS-140922
...
neibor(X):-   
     (isat(_,X);                     
      nostation(X) ;  
      unproperstation(X) ; %% recognised as neighbourhood (feature)       %% From tuc/lex.pl  
%     unproperstation1(X) ; %% recognised as neighbourhood (feature)       %% From tuc/lex.pl
      %% maybe empty
