...
rewordrepl1([A|XL],AAR,1):- % only 1 replacement
   reword1d([A|AZ],XZ:AAR),  %% dictn/e  
...
reword1d([A|AZ],XZAAR):- %% TA-101006
    reword1([A|AZ],XZL),
    make_dlist(XZL,XZAAR).
...
tsxrepl1([A|XL],AAR,1):- % only 1 replacement
   rw1([A|AZ],XZ:AAR),
   tsxmatches(AZ,XL,XZ),
   !.
