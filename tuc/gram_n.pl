...
%% Det 'spørsm''ålet' er brukerne interessert i 'å' stille
sentence1(new:::P)  --->  %%  
...
%% Det 'spørsm''ålet' er brukerne interessert i
...
substatement1(P)  --->  %%  (bussen 'går' selv om) det er nardo kunden * 'spør' om . 
     ...
     np0_accept(Z,ZNP), %% slike 'spørsm''ål'
     optional([som]),
     subject(X, XNP),     
     !,
     statreal(P) - 
...
statem(S,STM) --->     %% 'spørsm''ålet' har jeg svart 'på'.
...
prestatem(S,STM) ---> %% da skal bussen 'kjøres' av en 'sjåf''ør' 
...
itstatem(S,Com::P) ---> %% Det er godt jeg lever
    ...
    saa0,
    w(adj2(Good,nil)),
   {testmember(Good,[good])},  %%  Er det edru 'sjåf''ører'
...
whatq(which(X:thing):::D) --->  %% hva er det du snakker om
     whatbe,  
     [det],
     not_look_ahead(w(adj2(_,nil))), %% det store 'spørsm''ålet'
     subject(Y,NP),
     negation0(_),
     !,
...
howadjq(P) ---> % hvor mye tid/lang tid/  tar det('kjøret''øyet') fra
    hvor,       %                            
    howadj1(Noun), % hvor lang tid 
    {testmember(Noun,[duration])}, 
    w(verb(Take,pres,fin)), 
...
vp_kernel(Give,X,N,  S,Com3::(P1 and Q1)) ---> %% TA-101207 FRONTEST
    ...
    optional([til]), %% ad hoc  Garden path %%   
                     %% hvilken dame fortalte 'sjåf''øren' deg 'å'  gi billetten (til) ?   
...
% Hvilke 'spørsm''ål' har du tenkt 'å'  motta %% EXPENSIVE (Ad Hoc)
qverb_phrase(Y,N, S, ComP2 ) --->
    has,
    np1_accept(X, P::P1),    % du
...
% Hvilke 'spørsm''ål' har du mottatt %% EXPENSIVE 
qverb_phrase(Y,N, S, ComP2 ) --->
    has,
    np1_accept(X, P::P1),  % du
...
nil_compl(Tell) ---> %% har 'spørsm''ål' hver dag %% special 
    gmem(Tell,[have]),
    look_ahead_lit([hver]),
    !.
...
%% du sa til meg i stad at du ikke kunne svare 'på' 'spørsm''ålet'.
...
%% particlev2(put,expel)    ---> [av] %%  no bli satt_av av 'sjåf''øren'
...
aname_phrase(_IND,_,__) ---> %% er 'sjåf''øren' 'på' 36  blid
    number(_),                %% 36
...
npa(X, VP::P) ---> %%  'når' 'går' (den/det) neste til TS
    ...
    not_look_ahead([f,s,'først',sist,senest,tidligst]), %% TA-101029
                   %% bare s:  ikke det siste 'kjøret''øyet'
    flnp(Adj),   
       not_look_ahead(w(name(_,_,_))), %% 1. 'nyttårsdag'
       not_look_ahead(w(noun(_,_,_,_))), 
    !,
...
noun_modsx0(0,X:T,P1,        %% ('spørsm''ål') om hvor viktig det er
        P1  and NewS isa coevent and BINGO and P2) ---> 
...
agent_pronoun ---> [jeg].
agent_pronoun ---> [du]. %% er det du som er 'sjåf''ør' 
...
someone ---> [en],  %% HAZARDOUS destroys   en 'sjåf''ør'
    look_ahead([som]).
...
grums ---> w(noun(question,sin,u,n)),[':']. %%\+ opt.   %% \+ 'spørsm''ålet' ...
grums ---> [svar],optional([':']). 
