...
:-module( extracut, [ create_regcut/1 ] ). %% RS-150104 Moved to individual rModule/regcut.pl files: 
%%      ex_cutloop_rid/7, ex_cutloop_trace/3, ex_departureday/4, ex_ntourstops/2, ex_passes4/6, mod_cut_rid/8, pax/4, whattoget/7 ] ).
...
:-volatile ex_cutloop_rid/7,    %% RS-140914 Is it safe to make these volatile (not written to disk? with save_program?) Yes, they are written to rModule/regcut.pl instead...
          ex_cutloop_trace/3,
...
create_regcut(Module) :- 
   ...
   nl,out('Creating '),out(Module),out('/regcut.pl...'),nl,
   ...
   out('%% Comment generated by utility/extracut.pl æøå. To recreate: compile_route_set( '), out(Module), output(' )'),
   out('%% Or compile_route_set( '), out(Module), output(' ),use_module( ''utility/extracut.pl'', [ create_regcut/1 ] ), create_regcut( M )'),
   out(':- module( '),out(Module),output(', [ composite_stat/3, dko/8, dkodate/2, ex_departureday/4, ex_ntourstops/2, ex_passes4/6, hpl/4, regbus/1, route/3, xi/2 ] ).'),
   out('% RS-2019.08.09 Not needed after line 22, 30, 36, 60, 66 etc. stopped circling : cutloop_trace/3, cutloop_rid/7'),
