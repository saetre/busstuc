...
%% Some modifications to print pure list bracketed syntax trees 


:-module(drucke_baum, [ drucke_baum_list/1, lag_fil/1, member_x/2, term_laenge/2 ]). %% 

%:- ensure_loaded( utility ). %% Somehow puts utility in a module??!
...
tabx(X) :- %% Sictus Doesn take tab(-1) %% TA-061030
    ( X <0 -> true ; tab(X)).


%% Convert lists to terms %% TA-061027
...
atomize1(LitG,G):-  
    LitG =..[Lit,F1|_], %% verb(love,pres,fin) -> verb(love)
    !,
    atomize1([Lit,F1],G).
...
% run:-drucke_baum(s(np(det(the),n(man)),vp(v(sees),pp(p(in),np(det(the),n(house)))))).

% run1:-drucke_baum([dette,[er,en,liste]]).
...
knoten_zahl([k(_,_)|L],1):- xnot(member_x(k(_,_),L)).
