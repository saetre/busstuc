...
%:-module( checkitem, [ checkanswer/5, checkitem/3, current_frame/1, sysout_item/1 ] ).
%
%%% lche :- load_files('dialog/checkitem.pl').
%
%%:- ensure_loaded('../utility/utility'). %% [roundmember/2 etc.]
%:- use_module( '../utility/utility', [ for/2, roundmember/2 ] ).
%%% :- ensure_loaded( user:'../declare' ).
%
%%% UNIT /
%%:- ensure_loaded( user:'../utility/utility' ). %, [ := /2 etc. ] ).  %% RS-131117 includes declare.pl
%:- use_module( '../utility/utility', [ roundmember/2 ] ). %% RS-140208. Includes user:declare, and GRUF (fernando) %% :-op( 714,xfx, := ).
%
%%%% RS-131225, UNIT: /
%%:- use_module( '../main', [ trackprog/2 ] ).
%
%%%% RS-140914, UNIT: /app/
%:- use_module( '../app/interapp', [ isuccess/1, makeanswer/4, writeanswer/1 ] ).
...
%:- use_module( newcontext2, [ addref/3, commitref/3, getcontext/2, getcurrent/1, setcontext/2 ] ). %% RS-140101
%%:- use_module( parseres, [ busanswer_sat/3, listrequirements/1, teleanswer_sat/2, writetelebusteranswer_rep/1, writetelebusteranswer_saf/2, writetelebusteranswer_sqt/3, writetelebusteranswer4/4 ] ). %% Printing the result from database query 
%:- use_module( update2, [ dorelax/3, getuserrefer/2, issubclass/2, istype/2, saturate/3, saturatemod/3, updateframe/3, updateframe_checkanswer/4 ] ).
%
